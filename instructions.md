...
Weekly meal-planning agent powered by a LLM and driven by Telegram. Generates 4 seasonal, balanced, mostly vegetarian meal ideas each week, delivered on Telegram with a consolidated shopping list. The user can request a recipe replacement via the `/remplace` command, or replace a recipe with a dish of their choice via `/remplacepar`.
...
- **`/remplace <recette>`** — fuzzy-matches the named recipe in the current menu, asks the LLM for a season-compatible replacement, updates `menus.json`, and sends back the updated menu.
- **`/remplacepar <recette> | <souhait>`** — same as `/remplace`, but the replacement is based on the dish requested after the `|` (the LLM may refine the name and fill in the ingredients).
- **Anything else** — sends a usage hint listing the available commands.

During both generation and replacement, the LLM also receives the list of recipes from the last 8 weeks (extracted from `menus.json`) to avoid re-proposing the same dishes.
